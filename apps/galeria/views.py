...
def index(request):
    if not request.user.is_authenticated:
        messages.error(request, "Não é possível acessar essa página sem estar logado!")
...
def filtro(request, categoria):
    fotografia =  Fotografia.objects.order_by('data_foto').filter(publicada=True, categoria=categoria)
